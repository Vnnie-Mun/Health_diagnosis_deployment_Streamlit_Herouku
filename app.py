# -*- coding: utf-8 -*-
"""
Created on Mon Jul  7 15:16:08 2025
...
diabetes_model = pickle.load(open('C:/Users/pc/Downloads/Multiple Disease predictive system/Diabetes_trained_model.sav', 'rb'))
parkinsons_model = pickle.load(open('C:/Users/pc/Downloads/Multiple Disease predictive system/parkinsons_disease_model.sav', 'rb'))
heart_disease_model = pickle.load(open('C:/Users/pc/Downloads/Multiple Disease predictive system/Heart_disease_model.sav', 'rb'))
...
if selected == "Parkinsons Prediction":
    ...
    if st.button("Parkinson's Test Result"):
        try:
            input_data = [float(fo), float(fhi), float(flo), float(Jitter_percent), float(Jitter_Abs), float(RAP), float(PPQ),
                          float(DDP), float(Shimmer), float(Shimmer_dB), float(APQ3), float(APQ5), float(APQ),
                          float(DDA), float(NHR), float(HNR), float(RPDE), float(DFA), float(spread1), float(spread2),
                          float(D2), float(PPE)]
